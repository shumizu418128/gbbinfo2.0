...
import gbbinfojpn.modules.gemini as gemini
from gbbinfojpn.modules.participants import (create_world_map, get_participants_list,
                                             search_participants, instagram)
from gbbinfojpn.modules.result import get_result
...
for year in available_years:
    valid_categories = pd.read_csv(
        f'gbbinfojpn/static/csv/participants/{year}.csv')["category"].unique().tolist()
...
for year in available_years:
    ...
    try:
        all_category = os.listdir(f"./gbbinfojpn/static/csv/result/{year}")
    except Exception:
...
for year in available_years:  # 利用可能な年度をループ
    ...
    contents = os.listdir(f"./gbbinfojpn/templates/{year}")
...
content_others = os.listdir("./gbbinfojpn/templates/others")
